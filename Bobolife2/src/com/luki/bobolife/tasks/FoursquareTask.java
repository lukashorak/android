...
import com.luki.bobolife.model.FoursquareLogItem;

public class FoursquareTask extends AsyncTask<FoursquareLogItem, Void, String> {

	private static final String TAG = DataPostTask.class.getSimpleName();

	public FoursquareTask(Context context) {
	...
	@Override
	protected String doInBackground(FoursquareLogItem... vars) {
		Log.i(TAG, "Start");
		...
		String data = null;
		try {
			String url = "http://bobolife-gae.appspot.com/foursquareupdate";
			...
			String spending = "" + vars[0].getSpending();
			String items = Arrays.toString(vars[0].getItemsBought());
			nameValuePairs.add(new BasicNameValuePair("update", json));
			nameValuePairs.add(new BasicNameValuePair("spending", spending));
